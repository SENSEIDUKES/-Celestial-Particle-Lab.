...
export interface DevLoadingVeilCardProps {
...
/**
 * DEV copy of LoadingVeil — under active iteration for Versa's floating
 * emblem. Diverges from the reference (LoadingVeil.tsx) so the aura work
 * can be judged side by side in the Workshop:
 * - a grounded violet pool beneath the figure, breathing independently of
 ...
 */
export default function DevLoadingVeilCard({ task, onMinimize, backdrop, emblemClassName }: DevLoadingVeilCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 w-36 h-36 sm:w-40 sm:h-40 ${emblemClassName ?? 'mb-10 mt-4'} flex items-center justify-center shrink-0`}>
        ...
        {/* Ground pool — a grounded shadow that doesn't rise with her, so the hover above it reads as levitation. */}
        {isVersa && (
          <motion.div
            aria-hidden="true"
