...
import { CosmicArtifact } from '../relics/types';

/**
 * Workshop replica of the full-screen Relic Reveal / artifact celebration flow
 * from Light-Novels `src/components/ModalsAndToasts.tsx` (the
 * `unlockedArtifactAlert` AnimatePresence block).
 *
 * Production systems intentionally removed:
 * - `useAppStore` reveal queue (`enqueueRelicReveal`, `popPendingRelic`, reader gating)
 * - story-engine wiring and dismissal side effects
 *
 * Presentation is preserved 1:1 — rarity themes, effect ladder, sigil SVG,
 * drift motes, reveal flare, shimmer, claim button, and reduced-motion handling.
 */
