...
- **2026-08-06:** Standardized the primary creation language on **Manifest** and moved both generation buttons onto the new shared `ManifestButton` from the Library component set (the spectral portal → violet → gold rim shared with `LibraryPanel`, over obsidian; same behavior base and size scale as `LibraryButton`, so idle, hover, focus, loading, disabled, and mobile states match the rest of the Library). "Forge World Blueprint" / "Forge" became **Manifest World Blueprint** / **Manifest** in the intake action bar, and the Blueprint review's bare "Manifest" became **Manifest Story**; loading copy now reads "Manifesting..." (the VERSA drafting/writing swaps are unchanged). Secondary actions (Refine Details, Copy Blueprint, Export, Save, Import) keep their quieter `LibraryButton` variants and normal names, and the seed-library empty state now reads "Manifest or import one". Layouts untouched.
