import React, { forwardRef, ReactNode, useId } from 'react';
import type { LucideIcon } from 'lucide-react';

/**
 * LibraryTextArea — the official Celestial Library multi-line text field.
 *
 * The textarea counterpart of `LibraryTextBox`, built on the same rules.
 * Behavior is ported from SEIHouse UI's `SEITextarea` / `SEIField`
 * (UI repo: `packages/seihouse-ui/src/forms/`, ported 2026-08-03 — only the
 * text-field behavior, not the whole package):
 *
 * - `forwardRef` to the underlying `<textarea>`
 * - `useId`-generated fallback id, so `label htmlFor` and message ids always wire up
 * - `aria-describedby` wiring — the error message wins over the helper text,
 *   and any caller-supplied `aria-describedby` is merged in, never clobbered
 * - `aria-invalid` driven by `invalid` or a present `error`
 * - the error message carries `role="alert"` so screen readers announce it
 *   when it appears
 * - required marker (visual `*` plus an sr-only "(required)")
 * - `size` variants: `comfortable` (default) and `compact` — both use 16px
 *   text on phones so iOS Safari does not auto-zoom on focus; compact keeps
 *   its dense 12px text from the sm breakpoint up
 * - icon-bearing surface wrapper (`glass-field-wrap` + `glass-field-icon`,
 *   icon pinned to the field's top-left edge)
 * - a live character counter in the label row when `maxLength` is set
 *   (controlled mode only — uncontrolled fields cannot track the count)
 * - `children` render inside the field wrapper for overlays (e.g. the
 *   Premise ghost-tag button)
 *
 * Controlled or uncontrolled: pass `value` + `onChange` to drive it from
 * state, or omit `value` and let the browser manage the text (`defaultValue`,
 * `onBlur`, and `ref` all work natively). The quiet completed accent only
 * applies in controlled mode, where the current text is known.
 *
 * The skin stays entirely on the Library glass field system (`glass-field.css`).
 *
 * Architecture rule: pages and workspaces import `LibraryTextArea` — never a
 * raw `<textarea>` or a page-specific field. If a surface needs a different
 * visual mood, add a `variant` value here instead of creating a new component.
 */
export interface LibraryTextAreaProps
  extends Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, 'value' | 'onChange' | 'size'> {
  /** DOM id. Optional — a stable id is generated when omitted. Preview scripts rely on explicit ids, so keep passing them where they exist. */
  id?: string;
  label?: ReactNode;
  /** Controlled value. Omit for uncontrolled usage (`defaultValue` then applies). */
  value?: string;
  onChange?: (value: string) => void;
  /** Guidance rendered between the label and the field. */
  helpText?: ReactNode;
  /** Error message rendered under the field; also marks the field invalid and is announced to screen readers. */
  error?: string;
  /** Small node rendered at the right end of the label row, after the character counter. */
  rightElement?: ReactNode;
  /** Small contextual icon resting inside the field's top-left edge. */
  icon?: LucideIcon;
  /** Marks the field invalid without a message (keeps the glass surface, warning-colored edge). */
  invalid?: boolean;
  /** `comfortable` is the default; `compact` matches the character/faction grid fields. */
  size?: 'comfortable' | 'compact';
  /** Visual mood. Only the Celestial glass skin exists today — add new moods here, never as a new component. */
  variant?: 'glass';
  /** Overlay content rendered inside the field wrapper (e.g. a floating action button). */
  children?: ReactNode;
}

const sizeClasses: Record<NonNullable<LibraryTextAreaProps['size']>, string> = {
  comfortable: 'resize-none p-3 text-base',
  compact: 'resize-none px-2.5 py-1.5 text-base sm:text-xs',
};

export const LibraryTextArea = forwardRef<HTMLTextAreaElement, LibraryTextAreaProps>(
  function LibraryTextArea(
    {
      id,
      label,
      value,
      onChange,
      helpText,
      error,
      rightElement,
      icon: Icon,
      invalid = false,
      size = 'comfortable',
      variant = 'glass',
      required = false,
      disabled = false,
      className = '',
      maxLength,
      children,
      'aria-describedby': ariaDescribedBy,
      ...rest
    },
    ref,
  ) {
    const generatedId = useId();
    const inputId = id ?? generatedId;
    const helperId = `${inputId}-helper`;
    const errorId = `${inputId}-error`;

    const hasError = Boolean(error);
    const isInvalid = invalid || hasError;
    const describedBy =
      [hasError ? errorId : helpText ? helperId : undefined, ariaDescribedBy]
        .filter(Boolean)
        .join(' ') || undefined;

    const controlled = value !== undefined;
    const complete = controlled && !isInvalid && Boolean(value?.trim());
    const showCounter = maxLength !== undefined && controlled;

    const compact = size === 'compact';

    return (
      <div data-disabled={disabled || undefined}>
        {(label != null || rightElement || showCounter) && (
          <div className={`flex flex-wrap justify-between items-end gap-x-3 gap-y-1 ${compact ? 'mb-1' : 'mb-2'}`}>
            <label
              className={`block flex gap-2 items-center font-sc text-xs ${compact ? 'sm:text-[10px]' : ''} text-neutral-400 uppercase tracking-widest ${disabled ? 'opacity-45' : ''}`}
              htmlFor={inputId}
            >
              {label}
              {required && (
                <>
                  <span aria-hidden="true" className="text-human">*</span>
                  <span className="sr-only">(required)</span>
                </>
              )}
            </label>
            <div className="flex shrink-0 items-center gap-3 ml-auto">
              {showCounter && (
                <span className="whitespace-nowrap text-[10px] font-mono text-neutral-500">
                  {(value || '').length} / {maxLength}
                </span>
              )}
              {rightElement && <span>{rightElement}</span>}
            </div>
          </div>
        )}

        {helpText && (
          <p
            id={helperId}
            className={`text-neutral-500 font-sans text-xs ${compact ? 'mb-2' : 'mb-3'} leading-relaxed`}
          >
            {helpText}
          </p>
        )}

        <div className={variant === 'glass' ? 'glass-field-wrap' : ''}>
          {Icon && (
            <Icon
              size={compact ? 13 : 15}
              aria-hidden="true"
              className={`glass-field-icon ${compact ? 'top-[0.6rem]' : 'top-[1rem]'}`}
            />
          )}
          <textarea
            ref={ref}
            id={inputId}
            {...(controlled ? { value: value ?? '' } : {})}
            onChange={(e) => onChange?.(e.target.value)}
            disabled={disabled}
            required={required}
            maxLength={maxLength}
            aria-invalid={isInvalid || undefined}
            aria-describedby={describedBy}
            data-complete={complete || undefined}
            data-invalid={isInvalid || undefined}
            className={`glass-field ${sizeClasses[size]} ${Icon ? (compact ? 'pl-8' : 'pl-10') : ''} ${className}`}
            {...rest}
          />
          {children}
        </div>

        {hasError && (
          <p id={errorId} role="alert" className="mt-2 font-sans text-xs text-human">
            {error}
          </p>
        )}
      </div>
    );
  },
);
