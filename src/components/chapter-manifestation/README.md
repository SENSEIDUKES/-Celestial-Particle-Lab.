...
development/journey-scrubber/trails.tsx              — aura trail preset registry (qi-glow, mist-trail, starlight-trail)
...
- **Aura trails:** the completed-trail rendering is a preset system in `journey-scrubber/trails.tsx`, selected with the `trailStyle` prop. Presets are picked by stable id — never free-form config. Registered presets: `qi-glow` (default — the clean violet light trail), `mist-trail` (wider, softer, diffuse drifting energy with a slow shimmer), `starlight-trail` (subtler base line with tiny twinkling celestial sparks along the lit portion). Geometry is identical across presets — same curve, viewBox, and dimensions; a preset only styles the traveled portion (clip with the `pathLength` dash trick). Unlit path, milestone motes, gate, status text, and the indeterminate drift stay shared. Under reduced motion, mist drift and spark twinkle become static and calm.
...
- `development/LoadingVeilCard.tsx`, `development/LoadingSystem.tsx`, `development/AILoadingVeil.tsx`, `development/ManifestationChamber.tsx`, `development/SwordCultivatorClash.tsx`, `development/CelestialChannel.tsx`, and the full `development/journey-scrubber/` folder (scrubber, three travelers, traveler registry, trail presets — once approved, transfer as the new reference implementation)
...
- The journey scrubber expects a normalized 0–1 `progress` prop; keep the caller-side normalization (`task.progress / 100`) when transferring. Pass `travelerId` / `trailStyle` only with ids registered in `travelers.ts` / `trails.tsx` — unknown ids fall back to `cultivator` + `qi-glow`. A new traveler is one component honoring `TravelerRenderProps` plus one registry entry; a new trail preset is one component honoring `TrailRenderProps` (shared geometry, lit portion only) plus one registry entry. Reduced-motion fallbacks are each component's own responsibility.
